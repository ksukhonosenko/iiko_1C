...
Const ForWriting = 2
Const fileName = ".\test_Transformations.csv"
...
ts.WriteLine "Дата;Номер;Проведен;Склад списания(код);Склад списания (наименование);Склад прихода(код);Склад прихода (наименование);" & _
        "Знак операции;Знак операции (название);Номенклатура (код);Номенклатура (наименование);Группа учета (название);Группа учета (код);" & _
...
Set oDocuments = loader.LoadTransformationsData2("http://localhost:8080/resto/", "admin", "resto#test", filter)

Set oDocument = oDocuments.GetFirstTransformation()

Do While Not oDocument Is Nothing
    PrintDocument(oDocument)
    Set oDocument = oDocuments.GetNextTransformation()   
Loop
